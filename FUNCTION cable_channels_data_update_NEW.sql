CREATE OR REPLACE FUNCTION public.cable_channels_data_update_new() RETURNS trigger AS $cable_channels_data_update_new$ 

--!!!!!! использую для замены оригинальной функции public.cable_channels_data_update -  эту нужно удалить после запуска новой ! ------------ 
-- триггер нужно поставить только на GEOM  
-- триггер на таблицы [city]_cable_channels
...
---------привязка домов к концам кабеля  (подъезды обновляем отдельно) ------
     ...
     IF NEW.geom_cable IS NOT NULL THEN  
      ...
      EXECUTE 'SELECT CASE WHEN count(*) > 0 THEN TRUE ELSE FALSE END FROM '||tbl_build||' WHERE ST_Distance(ST_EndPoint($1.geom_cable) , '||tbl_build||'.building_geom) <= 4.1 ' INTO geom_end_point_state_house USING NEW;
        ...
        EXECUTE 'UPDATE  '||tbl_UPD||'
        SET
         cubic_start_street = ''_кабель_без_геометрії'',
         cubic_end_street = ''_кабель_без_геометрії''
        
         WHERE geom_cable IS NULL AND  tu_number IS NOT NULL AND rental_contract_new_num IS NOT NULL AND   contract_chanel_length IS NOT NULL ;'USING NEW;   -- попытка выделить кабели залитые но не нарисованы
...
--вот эта часть конфликтутовала всё затирает NULL при изменении геометрии
-- добавил дополнительно  IF NEW.geom_cable IS NOT NULL THEN  - и это помогло 
   IF NEW.geom_cable IS NOT NULL THEN  
    EXECUTE 'UPDATE '||tbl_UPD||'
        ...
        */
        /*найти скрипт который заливает csv и туда добавить "ВОК без геометрии"*/
...
----------------------------------------------- 
  ...
  END IF; 
END;
$cable_channels_data_update_new$ LANGUAGE plpgsql;
