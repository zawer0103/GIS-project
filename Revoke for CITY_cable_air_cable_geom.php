...
// http://10.112.129.170/qgis-ck/php/test-zawer/Revoke for CITY_cable_air_cable_geom.php
...
if (is_array($cities) || is_object($cities))
{
 ...
 $revoke =" 
REVOKE ALL ON ".$selectedCity.".".$selectedCity."_cable_air_cable_geom FROM simpleuser;
REVOKE ALL ON ".$selectedCity.".".$selectedCity."_cable_air_cable_geom FROM simplereader;
 " ;  $result=pg_query($postgres, $revoke); 


#ALTER TABLE ".$selectedCity.".".$selectedCity."_cable_air_cable_geom DISABLE TRIGGER cable_channels_data_update

$alter ="ALTER TABLE ".$selectedCity.".".$selectedCity."_cable_air_cable_geom DISABLE TRIGGER cable_air_data_update;";
    $result=pg_query($postgres, $alter); 

$alter ="
 COMMENT ON TABLE  ".$selectedCity.".".$selectedCity."_cable_air_cable_geom  IS 'не используем: перешли на вьюшку'";
    $result=pg_query($postgres, $alter); 
