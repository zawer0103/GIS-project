...
if (is_array($cities) || is_object($cities))
{
  ...
  // ".$selectedCity.".".$selectedCity."_
    ...
    $alter =" ALTER TABLE ".$selectedCity.".".$selectedCity."_cable_channels_coax  ADD CONSTRAINT unic_id  UNIQUE  (table_id);  ";
    $result=pg_query($postgres, $alter); 
    
   $alter =" ALTER TABLE ".$selectedCity.".".$selectedCity."_cable_channel_coax_geom ADD CONSTRAINT table_key  FOREIGN KEY (table_id) REFERENCES ".$selectedCity.".".$selectedCity."_cable_channels_coax  (table_id);  ";
    $result=pg_query($postgres, $alter); 
   ...
   $alter =" ALTER TABLE ".$selectedCity.".".$selectedCity."_cable_channel_coax_geom ADD CONSTRAINT table_key_coax  FOREIGN KEY (table_id) REFERENCES ".$selectedCity.".".$selectedCity."_cable_channels_coax (table_id);  ";
    ...
    # нужно добавить в таблицах _cable_channels_coax    для поля table_id  добавить unique key 

     ///ALTER TABLE fastiv.fastiv_cable_channels_coax  ADD CONSTRAINT unic_id  UNIQUE  (table_id); // протестил ОК
    ///ALTER TABLE fastiv.fastiv_cable_channel_coax_geom ADD CONSTRAINT table_key  FOREIGN KEY (table_id) REFERENCES fastiv.fastiv_cable_channels_coax  (table_id); // протестил ОК

    #  у нас какие-то лишние таблицы везде: _cable_channels_cable_geom
    #  в QGIS  у нас подключены  _cable_channel_coax_geom    (без буквы s)
