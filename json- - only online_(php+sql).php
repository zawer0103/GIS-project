...
$create_hybrids_online ="create table IF NOT EXISTS ".$selectedCity.".".$selectedCity."_hybrids_online (city varchar(20), house_id  varchar(20), street varchar(100), house varchar(20), sector varchar(20), flat varchar(20), mac varchar(20), serial varchar(20), technology_name varchar(20), link varchar(100), equipment_geom geometry,   equipment_level varchar(20), equipment_snr varchar(20), equipment_frequency varchar(20), equipment_modulation varchar(20),  equipment_update_time timestamp with time zone);";
$result=pg_query($postgres, $create_hybrids_online); //ВНИМАНИЕ !!!  в json поле называеться mac а в БД КУБИК - это поле называеться serial (см таблицу [city]_hybrids) )
$clear_hybrids_online ="DELETE FROM ".$selectedCity.".".$selectedCity."_hybrids_online;";
$result=pg_query($postgres, $clear_hybrids_online); //очистка таблиц онлайн   (каждые 15мин)

$insert_hybrids_online ="INSERT INTO ".$selectedCity.".".$selectedCity."_hybrids_online (serial, equipment_level, equipment_snr, equipment_frequency,  equipment_update_time) SELECT TMP.mac, TMP.level, TMP.snr, TMP.freq, to_timestamp(TMP.ts)::timestamp with time zone FROM TMP WHERE TMP.mac IN (SELECT ".$selectedCity.".".$selectedCity."_hybrids.serial FROM ".$selectedCity.".".$selectedCity."_hybrids); ";  ///  потом(когда Яценко добавит в json) нужно добавить поле modulation  //ВНИМАНИЕ !!!  в json поле называеться mac а в БД КУБИК - это поле называеться serial (см таблицу [city]_hybrids) )
$result=pg_query($postgres, $insert_hybrids_online); //вставляем данные мониторинга  в таблицу [city]_hybrids-online (каждые 15мин) 

$update_hybrids_online ="UPDATE ".$selectedCity.".".$selectedCity."_hybrids_online set city=".$selectedCity.".".$selectedCity."_hybrids.city, house_id=".$selectedCity.".".$selectedCity."_hybrids.house_id, street=".$selectedCity.".".$selectedCity."_hybrids.street, house=".$selectedCity.".".$selectedCity."_hybrids.house, sector=".$selectedCity.".".$selectedCity."_hybrids.sector, flat=".$selectedCity.".".$selectedCity."_hybrids.flat,mac=".$selectedCity.".".$selectedCity."_hybrids.mac, technology_name=".$selectedCity.".".$selectedCity."_hybrids.technology_name, equipment_geom=".$selectedCity.".".$selectedCity."_hybrids.equipment_geom, link=".$selectedCity.".".$selectedCity."_hybrids.link                 FROM ".$selectedCity.".".$selectedCity."_hybrids WHERE ".$selectedCity.".".$selectedCity."_hybrids_online.serial=".$selectedCity.".".$selectedCity."_hybrids.serial;"; 
$result=pg_query($postgres, $update_hybrids_online);//вставляем другие необходимые поля  в таблицу [city]_hybrids-online (каждые 15мин) из таблиц  [city]_hybrids
