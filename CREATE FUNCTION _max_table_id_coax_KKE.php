...
////table_id = NULL  
$fun =" CREATE OR REPLACE FUNCTION ".$selectedCity.".".$selectedCity."_max_table_id_KKE_coax() RETURNS varchar(7) LANGUAGE SQL AS
  $$ SELECT 't_'||left('00000',(5-length(tmp.id::varchar(5))))||tmp.id FROM (SELECT CASE when max(right(table_id, 5)::int) is NULL THEN 1 else max(right(table_id, 5)::int)+1 end as id from  ".$selectedCity.".".$selectedCity."_cable_channel_coax_geom) tmp; $$;
  Alter table ".$selectedCity.".".$selectedCity."_cable_channel_coax_geom
  Alter  COLUMN  table_id SET DEFAULT ".$selectedCity.".".$selectedCity."_max_table_id_KKE_coax();
  Alter table ".$selectedCity.".".$selectedCity."_cable_channel_coax_geom
  Alter  COLUMN  cable_type SET DEFAULT 'coax';   " ;
