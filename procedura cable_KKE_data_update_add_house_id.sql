...
--!!!!!! протестировал использую для  замены оригинальной функции cable_channels_data_update  !!!!!!!!!!!!! ------------
DECLARE 
  ...
  --оригинальная функция cable_channels_data_update () 
...
---[city]_cable_channel_cable_geom (0шт) --- лажа какая-то лишняя буква s - нужно поудалаять со всех таблиц


            EXECUTE 'UPDATE '||city||'.'||city||'_cable_channels
            SET
              cable_short_type_description = '||city||'_cable_channel_cable_geom.cable_short_type_description
            FROM '||city||'.'||city||'_cable_channel_cable_geom
            WHERE  '||city||'_cable_channels.table_id = '||city||'_cable_channel_cable_geom.table_id and '||city||'_cable_channels.cable_short_type_description is NULL;
            UPDATE '||city||'.'||city||'_cable_channels
            SET
              cable_description = '||city||'_cable_channel_cable_geom.cable_description
            FROM '||city||'.'||city||'_cable_channel_cable_geom
            WHERE  '||city||'_cable_channels.table_id = '||city||'_cable_channel_cable_geom.table_id and '||city||'_cable_channels.cable_description is NULL; 
            UPDATE '||city||'.'||city||'_cable_channels
            SET
              total_cable_length='||city||'_cable_channel_cable_geom.total_cable_length
            FROM '||city||'.'||city||'_cable_channel_cable_geom
            WHERE  '||city||'_cable_channels.table_id = '||city||'_cable_channel_cable_geom.table_id and '||city||'_cable_channels.total_cable_length is NULL;
            UPDATE '||city||'.'||city||'_cable_channels
            SET
              cable_type=DEFAULT
            WHERE  '||city||'_cable_channels.cable_type is NULL and '||city||'_cable_channels.cable_short_type_description is NOT NULL;'         USING NEW;
         EXECUTE 'UPDATE '||city||'.'||city||'_cable_channel_cable_geom
            SET
              cable_short_type_description = NULL,
              cable_description = NULL,
              total_cable_length = NULL
                        
            WHERE  '||city||'_cable_channel_cable_geom.cable_short_type_description is NOT NULL OR '||city||'_cable_channel_cable_geom.cable_description is NOT NULL OR '||city||'_cable_channel_cable_geom.total_cable_length is NOT NULL' USING NEW; /* затираем данные после сохранения вроде работает норм*/
      ...
      EXECUTE 'SELECT CASE WHEN count(*) > 0 THEN TRUE ELSE FALSE END FROM '||city||'.'||city||'_buildings WHERE ST_Distance(ST_EndPoint($1.geom) ,'||city||'_buildings.building_geom) <= 4.1 ' INTO geom_end_point_state_house USING NEW;
            IF geom_start_point_state_house = TRUE THEN
              EXECUTE 'UPDATE '||city||'.'||city||'_cable_channels
                  ...
                  WHERE  (ST_Distance(ST_StartPoint($1.geom),'||city||'_buildings.building_geom) <= 4.1 AND '||city||'_cable_channels.table_id = $1.table_id) ' USING NEW;
            ...
            IF geom_end_point_state_house = TRUE THEN
              EXECUTE 'UPDATE '||city||'.'||city||'_cable_channels
                  ...
                  FROM '||city||'.'||city||'_buildings
                  WHERE  (ST_Distance(ST_EndPoint($1.geom),'||city||'_buildings.building_geom ) <= 4.1 AND '||city||'_cable_channels.table_id = $1.table_id)  ' USING NEW , city;
...
--------------------------ниже добавляем обновляем подъезды к концам кабеля --------
            ...
            IF geom_start_point_state_house = TRUE THEN
              EXECUTE 'UPDATE '||city||'.'||city||'_cable_channels
                  ...
                  WHERE  (ST_Distance(ST_StartPoint($1.geom),'||city||'_entrances.geom) <= 6.1 AND '||city||'_cable_channels.table_id = $1.table_id) ' USING NEW;
            ...
            IF geom_end_point_state_house = TRUE THEN
              EXECUTE 'UPDATE '||city||'.'||city||'_cable_channels
                  ...
                  FROM '||city||'.'||city||'_entrances
                  WHERE  (ST_Distance(ST_EndPoint($1.geom),'||city||'_entrances.geom ) <= 6.1 AND '||city||'_cable_channels.table_id = $1.table_id)  ' USING NEW , city;
...
-------------------------
      ...
      IF geom_end_point_state = TRUE THEN
        ...
        FROM '||city||'.'||city||'_ctv_topology
        WHERE  (ST_Equals('||city||'_ctv_topology.equipment_geom ,ST_EndPoint($1.geom)) OR (ST_Distance(ST_EndPoint($1.geom) ,'||city||'_ctv_topology.equipment_geom) <= 2.8)) AND '||city||'_cable_channels.table_id = $1.table_id AND '||city||'_ctv_topology.cubic_name IN (''Магістральна ГС'', ''Магістральний оптичний вузол'', ''Магистральный распределительный узел'', ''Оптичний приймач'', ''Оптический узел'', ''Кросс-муфта'')  ' USING NEW , city;
      ELSE
        EXECUTE 'UPDATE '||city||'.'||city||'_cable_channels
            SET
              cubic_code_end = NULL,
...
-- тут есть некий дисонанс: если мы пытаемя обновить поля в слое "для рисования" система не даёт их перезаписать и в слоях кабели_ВКП_для_рисования и в кабели_ВКП будут разные данные - это будет вносить сумятицу: в связи с чем есть идея: затирать данные в таблице кабели_ВКП_для_рисования (скрипт добавил ниже)
        EXECUTE 'UPDATE '||city||'.'||city||'_cable_channels
            ...
            WHERE  '||city||'_cable_channel_cable_geom.cable_short_type_description is NOT NULL OR '||city||'_cable_channel_cable_geom.cable_description is NOT NULL OR '||city||'_cable_channel_cable_geom.total_cable_length is NOT NULL' USING NEW; /*  затераем данные в cable_air_cable_geom после сохранения вроде работает норм*/
...
-------------------------------------------------------------------------
    ...
    ELSIF TG_OP = 'DELETE' THEN 
        ...
        WHERE '||city||'_cable_channels.table_id = $1.table_id' USING OLD;
...
/*CREATE TRIGGER cable_KKE_house_id AFTER INSERT OR UPDATE OR DELETE ON fastiv.fastiv_cable_channel_cable_geom
    FOR EACH ROW EXECUTE PROCEDURE cable_KKE_house_id(); -- если сделать тригер before то нихрена не работает*/
    --[city]_cable_channel_cable_geom (0шт) --- лажа какая-то лишняя буква s - нужно поудалаять со всех таблиц и к ним даже процедуры привязаны   
    --ERROR:  column fastiv_cable_channel_cable_geom.cable_description does not exist